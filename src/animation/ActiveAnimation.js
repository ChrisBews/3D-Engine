class ActiveAnimation {
  ...
  update(elapsed) {
    ...
    this._easedProgress = Animber.Easing.getEasedPercentageOnCurve(this._options.easing, this._progress);
    if (this._isNumber) {
      this._updateNumberValue();
    } else {
      this._updateObjectValues();
    }
    if (this._options.onProgress) this.options.onProgress(this._progress);
